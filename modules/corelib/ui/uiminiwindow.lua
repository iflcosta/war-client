...
function UIMiniWindow:setup()
	...
	if self.miniwindowTopBar then
		function self.miniwindowTopBar.onDoubleClick()
			if self:isOn() then
				self:maximize()
			else
				self:minimize()
	...
	if self.bottomResizeBorder then
		function self.bottomResizeBorder.onDoubleClick()
	...
	if settings then
		...
		elseif not self.forceOpen and self.autoOpen ~= nil and (self.autoOpen == 0 or self.autoOpen == false) and not self.containerWindow then
			self:close(true)
		end
	end
