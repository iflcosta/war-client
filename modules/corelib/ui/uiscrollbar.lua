...
local function calcValues(self)
	...
	if self.orientation == "vertical" then
		...
		pxrange = self:getWidth() - decrementButton:getWidth() - decrementButton:getMarginLeft() - decrementButton:getMarginRight() - incrementButton:getWidth() - incrementButton:getMarginLeft() - incrementButton:getMarginRight()
		center = self:getX() + math.floor(self:getWidth() / 2)
	end

	local range = self.maximum - self.minimum + 1
	local proportion = nil
...
local function parseSliderPos(self, slider, pos, move)
	...
	if delta > 0 and self.hotDistance < hotDistance + delta or delta < 0 and hotDistance + delta < self.hotDistance then
		local range, pxrange, px, offset, center = calcValues(self)
...
function UIScrollBar:onMouseWheel(mousePos, mouseWheel)
	...
	if mouseWheel == MouseWheelUp then
		if self.orientation == "vertical" then
			...
			if self.maximum <= self.value then
				return false
			end
