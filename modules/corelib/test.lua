...
function Test.Test(name, func)
	...
	local function test(testFunc)
		table.insert(Test.tests[testId].actions, {
			type = "test",
			value = testFunc
		})
	end
...
function Test.run()
	...
	elseif action.type == "wait" and action.value + test.start < g_clock.millis() then
		table.remove(test.actions, 1)
	end
