		...
		for i = 1, format:len() do
			...
			elseif opt:find("[fd]") then
				...
				if opt == "d" then
					...
					for i = 1, 6 do
						table.insert(bytes, string.char(math.floor(val) % 256))

						val = math.floor(val / 256)
					end
				else
					table.insert(bytes, string.char(math.floor(mantissa) % 256))
				...
				table.insert(bytes, string.char(math.floor(exponent * (opt == "d" and 16 or 128) + val) % 256))

				val = math.floor((exponent * (opt == "d" and 16 or 128) + val) / 256)

				table.insert(bytes, string.char(math.floor(sign * 128 + val) % 256))

				val = math.floor((sign * 128 + val) / 256)
