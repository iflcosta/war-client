local overlay, keypad = nil
local touchStart = 0
local updateCursorEvent, zoomInButton, zoomOutButton, keypadButton, keypadEvent = nil
...
function init()
	...
	zoomInButton = modules.client_topmenu.addLeftButton("zoomInButton", "Zoom In", "/images/topbuttons/zoomin", function ()
		g_app.scaleUp()
	end)
	zoomOutButton = modules.client_topmenu.addLeftButton("zoomOutButton", "Zoom Out", "/images/topbuttons/zoomout", function ()
		g_app.scaleDown()
	end)
...
function updateCursor()
	...
	if not g_mouse.isPressed(MouseTouch) then
		return
	end

	local percent = 100 - math.max(0, math.min(100, (g_clock.millis() - touchStart) / 5))
...
function executeWalk()
	...
	if not dir and (math.abs(keypadMousePos.y - 0.5) > 0.1 or math.abs(keypadMousePos.x - 0.5) > 0.1) then
		if math.abs(keypadMousePos.x - 0.5) < math.abs(keypadMousePos.y - 0.5) then
			if keypadMousePos.y < 0.5 then
				dir = Directions.North
			else
				dir = Directions.South
			end
		elseif keypadMousePos.x < 0.5 then
			dir = Directions.West
		else
			dir = Directions.East
		end
