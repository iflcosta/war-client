...
function onGameEditText(id, itemId, maxLength, text, writer, time)
	for _, window in pairs(windows) do
		local textItem = window:getChildById("textItem")
		local textEdit = window:getChildById("text")
		local textSame = textEdit:getText() == text
		local textItemSame = textItem:getItemId() == itemId

		if textSame and textItemSame then
			window.onEscape()
		end
	end
