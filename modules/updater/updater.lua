...
local updaterWindow, loadModulesFunction, scheduledEvent = nil
...
function Updater.check(args)
	...
	local function progressUpdater(value)
		...
		if updateData and (value > 60 or not g_app.isMobile() or not ALLOW_CUSTOM_SERVERS or not loadModulesFunc) then
...
function Updater.changeUrl()
	removeEvent(scheduledEvent)
	modules.client_textedit.edit(Services.updater, {
		width = 300,
		title = "Enter updater url"
	}, function (newUrl)
