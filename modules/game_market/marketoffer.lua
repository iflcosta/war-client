...
function MarketOffer.new(offerId, t, item, amount, price, playerName, state, var)
	local offer = {
		item = 0,
		state = 0,
		amount = 0,
	...
	if state ~= MarketOfferState.Active and state ~= MarketOfferState.Cancelled and state ~= MarketOfferState.Expired and state ~= MarketOfferState.Accepted then
