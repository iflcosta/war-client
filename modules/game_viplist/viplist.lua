...
function onAddVip(id, name, state, description, iconId, notify)
	...
	for i = 1, childrenCount do
		...
		if state == VipState.Online and child.vipState ~= VipState.Online and getSortedBy() == "status" or child.iconId < label.iconId and getSortedBy() == "type" then
		...
		if (state ~= VipState.Online and child.vipState ~= VipState.Online or state == VipState.Online and child.vipState == VipState.Online) and getSortedBy() == "status" or label.iconId == child.iconId and getSortedBy() == "type" or getSortedBy() == "name" then
			...
			for j = 1, length do
				if nameLower:byte(j) < childText:byte(j) then
					...
					return
				elseif childText:byte(j) < nameLower:byte(j) then
					break
