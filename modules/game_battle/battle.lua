...
function checkCreatures()
	...
	local spectators = g_map.getSpectatorsInRangeEx(pos, false, math.floor(dimension.width / 2), math.floor(dimension.width / 2), math.floor(dimension.height / 2), math.floor(dimension.height / 2))
	...
	for _, creature in ipairs(spectators) do
		if doCreatureFitFilters(creature) and maxCreatures > #creatures then
...
function doCreatureFitFilters(creature)
	...
	elseif hideSkulls and creature:isPlayer() and creature:getSkull() == SkullNone then
		return false
	elseif hideParty and ShieldWhiteBlue < creature:getShield() then
		return false
	end
...
function sortCreatures(creatures)
	...
	if getSortType() == "distance" then
		...
		table.sort(creatures, function (a, b)
			if getDistanceBetween(playerPos, a:getPosition()) == getDistanceBetween(playerPos, b:getPosition()) then
				return ages[b:getId()] < ages[a:getId()]
			end

			return getDistanceBetween(playerPos, b:getPosition()) < getDistanceBetween(playerPos, a:getPosition())
		end)
	...
	if getSortType() == "health" then
		table.sort(creatures, function (a, b)
			...
			return b:getHealthPercent() < a:getHealthPercent()
		end)
	...
	elseif getSortType() == "screenage" then
		table.sort(creatures, function (a, b)
			return b.screenAge < a.screenAge
		end)
	else
		table.sort(creatures, function (a, b)
			...
			return b:getName():lower() < a:getName():lower()
		end)
	end
end
